...
def publish_all(date_str, draft_mode=False, headless=False, batch_filter=None):
    ...
    with sync_playwright() as p:
        ...
        # ============================================================
        # Publish micro-headlines (generated by orchestrator) if available
        # ============================================================
        try:
            from micro_headline import publish_micro_headlines
            # Check if orchestrator saved micro-headline data in metadata
            meta_path = OUTPUT_BASE / date_str / "metadata.json"
            if meta_path.exists():
                meta_data = json.loads(meta_path.read_text())
                stored_headlines = meta_data.get("micro_headlines", [])
                if stored_headlines:
                    print(f"\n📢 发布 {len(stored_headlines)} 条微头条...")
                    results = publish_micro_headlines(page, stored_headlines, date_str=date_str)
                    ok = sum(1 for r in results if r.get("ok"))
                    print(f"   📊 微头条: {ok}/{len(results)} 发布成功")
        except Exception as e:
            print(f"   ⚠️ 微头条发布异常: {e}")
